...
%--------------------------------------------------------------------------
    ...
    Anamesi = Anamesi(cellfun('isempty',A));
        ...
        Results.regl = zeros(Param.tleng-Param.tstart,1);   % number of subimages at each time
        for t = Param.tstart:Param.tleng
                reg = 0;
            ...
            for x = 1:rec:(Param.siz(2)- Param.pas2+1)    % register position
                for y = 1:rec:(Param.siz(1)-  Param.pas2+1)
                         reg = reg+1;
                         Results.Posi(reg,:,t-Param.tstart+1) = [floor(x),floor(y)];        
                end
            end
           Results.regl(t-Param.tstart+1) = reg;               % register number of subimages at each time
        end
        ...
        for t = Param.tstart:Param.tleng
           ...
           Results.regl(t-Param.tstart+1) = reg;
        end
    end
end