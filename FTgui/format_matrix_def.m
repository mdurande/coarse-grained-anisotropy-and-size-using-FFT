function [Udef,Vdef,Xdef,Ydef] = format_matrix_def(Param,Results,Row,Col)
...
%%
    ...
    %%
    for ml = 1:Results.ci
        for win =1:Results.regl(ml) % for each subimage
            [alk,blk] = find(Reg_xy == win);
          xo =  Results.Posi(win,1,ml)+Param.pas2/2-1; % find positions of the center of the subimages
          yo =  Results.Posi(win,2,ml)+Param.pas2/2-1;
            if sum(yo == Row)>0 && sum(Col(yo==Row)==xo)>0
                amp= NaN;
                ang=NaN;
            else
                amp = Results.im_regav(ml).c(win).S;    % extract amplitude 
                ang = Results.im_regav(ml).c(win).angS; % extract angle of deformation
